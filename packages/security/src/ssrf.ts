...
const BLOCKED_IPV6_RANGES: BlockedRange[] = [
...
export class SSRFError extends Error {
  constructor(message: string, public readonly blockedIP?: string) {
    super(message);
    this.name = 'SSRFError';
  }
}
 ...
 */
export async function validateUrlForSSRF(inputUrl: string): Promise<{ hostname: string; ips: string[] }> {
  let parsed: URL;
  try {
...
export async function safeFetch(url: string, options: SafeFetchOptions = {}): Promise<Response> {
  const {
    ...
    userAgent = 'RankForge/1.0 (+https://rankforge.io/bot)',
  } = options;
