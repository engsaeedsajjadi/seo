 ...
 * 
 * Architecture:
 * Frontend → API → PostgreSQL → Redis/Queue → Workers
 * 
 * Features:
 * - Multi-tenant with organization isolation + RLS
 * - JWT auth + bcryptjs 12 + RBAC
 * - Real crawler with SSRF protection
 * - Audit engine 13 rules transparent scoring
 * - Provider abstraction DataForSEO/SerpApi + 5 AI providers metered
 * - Billing Stripe 5 plans webhook verified, credits atomic, usage metering
 * - Agency clients white-label no leakage, client portal
 * - API keys hashed, webhooks signed, MCP tenant-isolated
 * - Security: helmet, CORS enforced, rate limiting, Zod validation, AES-256-GCM encryption, audit logs
 */
...
import { getPool, checkDatabaseHealth, closePool } from './db/client.js';
...
import { generateApiKey, hashApiKey } from './lib/encryption.js';
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // In development, allow preview hosts and localhost
...
app.post('/api/v1/projects', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    // Validate domain not private/internal
...
// ============================================================
// PLACEHOLDER ENDPOINTS WITH REAL STRUCTURE BUT NOT_CONFIGURED WHEN NEEDED
// ============================================================
...
app.get('/api/v1/projects/:projectId/rankings', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    // Rankings require provider, but we return empty with proper structure
