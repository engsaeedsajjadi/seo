...
export interface AuditContext {
  pages: Array<{
    ...
    links: Array<{ href: string; isInternal: boolean; rel?: string }>;
    responseTime: number;
...
export const imagesWithoutAltRule: AuditRule = {
  ...
  check: (ctx) => {
    ...
    if (pagesWithMissingAlt.length === 0) return null;
    return {
...
export const slowPagesRule: AuditRule = {
  id: 'slow_pages',
  name: 'Slow Pages',
...
export const insecureLinksRule: AuditRule = {
  id: 'insecure_links',
  ...
  check: (ctx) => {
    ...
    if (insecurePages.length === 0) return null;
    return {
...
export function runAudit(context: AuditContext): AuditFinding[] {
...
export function calculateSeoScore(findings: AuditFinding[]): { overall: number; categories: Record<Category, number> } {
  ...
  const allCategories: Category[] = ['crawlability', 'indexability', 'metadata', 'content', 'links', 'images', 'performance', 'security', 'structured_data', 'international'];
  for (const cat of allCategories) {
    if (!(cat in categories)) categories[cat] = 100;
  }

  return { overall: Math.round(score), categories: categories as Record<Category, number> };
}
